...
public class PlacesController : ControllerBase
{
    ...
    [HttpGet("nearby")]
    [ProducesResponseType(typeof(IEnumerable<NearbyPlaceDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ProblemDetails), StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<IEnumerable<NearbyPlaceDto>>> GetNearbyAsync(
        [FromQuery] double latitude,
        [FromQuery] double longitude,
        [FromQuery] double radiusMeters = 1000,
        CancellationToken ct = default)
    {
        var results = await _places.GetNearbyAsync(latitude, longitude, radiusMeters, ct);
        return Ok(results);
    }
